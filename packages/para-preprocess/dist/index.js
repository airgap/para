...
const HAS_BUN_TRANSPILER = typeof globalThis.Bun?.Transpiler === "function";
...
let _esbuildTransform;
function nodeStripTypes(src, loader) {
    if (_esbuildTransform === undefined) {
        try {
            ...
            _esbuildTransform = (s, l) => esbuild.transformSync(s, {
                loader: l,
                format: "esm",
                target: "esnext",
...
// Svelte semantics.
export function hasTopLevelAwait(body) {
    let depth = 0;
...
function declScan(s, isHit) {
    ...
    for (let i = 0; i < s.length; i++) {
        ...
        if (depth === 0 && isHit(i, prev, next))
            hits.push(i);
    }
    return hits;
}
/** Split a `prop`/`signal` declarator list on top-level commas. */
export function splitDeclarators(list) {
 ...
 */
export function stripDeclTail(tail) {
    let q = null;
 ...
 * `=>`, `==`, `===`, `<=`, `>=`, `!=`, `!==`, `+=` …).
 */
export function parseDeclarator(decl) {
 ...
 */
export function matchTypeStubSpans(source) {
...
function expandEffectSingle(src) {
    const re = /(^|[;\n{}])([ \t]*)effect[ \t]+(?=[A-Za-z_$])/g;
    let out = "";
    let last = 0;
    let m;
...
function lowerEffectBlocks(source) {
...
export const PUI_RUNTIME_LIFECYCLE = ["onMount", "onDestroy"]; // from the runtime (svelte / @lyku/para-ui)
export const PUI_KIT_NAV = ["beforeNavigate", "afterNavigate", "onNavigate"]; // from $app/navigation
...
export function usesIdentCall(src, name) {
    return new RegExp(`(^|[^\\w$.])${name}\\s*\\(`).test(src);
}
function lowerDerivedBlocks(source) {
...
// multi-line `derived = …`).
export function derivedInitEnd(src, start) {
    const contPrev = (c) => c !== "" && "+-*/%&|^<>=!~?:.,([{".includes(c);
    const contNext = (c) => c !== "" && "?:.,)]}+-*/%&|^<>=!([".includes(c);
    let i = start;
    let depth = 0;
    let lastSig = "";
    while (i < src.length) {
...
function lowerDerivedDecls(source) {
...
function lowerQueryDerivedDecls(source) {
...
function lowerPropDecls(source) {
    ...
    for (const li of declLines)
        if (li !== first)
            lines[li] = "";
    return lines.join("\n");
}
function lowerProvideInject(source) {
...
function lowerUsingDecls(source) {
...
function lowerSourceDecls(source) {
    ...
    const code = source.replace(re, (_full, indent, name, expr) => {
        needs = true;
        return (`${indent}const __src_${name} = ${stripDeclTail(expr)}; ` +
...
function syncedBinding(indent, name, call, type) {
    const ann = type ? `: ${type}` : "";
    return (`${indent}const __syn_${name} = ${call}; ` +
...
function lowerSyncFromDecls(source) {
...
function lowerSyncedDecls(source) {
...
function feedBinding(indent, name, call) {
    return (`${indent}const __syn_${name} = ${call}; ` +
        `let ${name} = $state(__syn_${name}.peek?.() ?? []); ` +
        `$effect.pre(() => __syn_${name}.subscribe?.((__v: typeof ${name}) => { ${name} = __v; })); ` +
        `onDestroy(() => __syn_${name}.dispose?.());`);
}
function lowerSyncFeedDecls(source) {
    ...
    while ((m = re.exec(source)) !== null) {
        ...
        while (source[consumeEnd] === " " || source[consumeEnd] === "\t")
            consumeEnd++;
        if (source[consumeEnd] === ";")
            consumeEnd++;
        out += source.slice(last, kwStart);
...
function lowerSyncOneDecls(source) {
...
function serverExprEnd(src, start, stmtEnd) {
    let i = start;
    let depth = 0;
    while (i < stmtEnd) {
        const c = src[i];
 ...
 */
export function extractServerSources(source, opts = {}) {
    ...
    while ((im = importRe.exec(source)) !== null) {
        const mod = im[4];
        if (im[1])
            importOf.set(im[1], mod);
        if (im[3])
            importOf.set(im[3], mod);
        if (im[2]) {
            for (const part of im[2].split(",")) {
                const name = part.trim().split(/\s+as\s+/).pop()?.trim();
                if (name)
                    importOf.set(name, mod);
            }
        }
    }
    ...
    while ((dm = declRe.exec(source)) !== null)
        componentNames.add(dm[1]);
    const syncRe = /(?:^|[;\n{}])\s*sync\s+([A-Za-z_$][\w$]*)/g;
    while ((dm = syncRe.exec(source)) !== null)
        componentNames.add(dm[1]);
    const spans = [];
    while ((m = re.exec(source)) !== null) {
        const kwStart = m.index + m[1].length;
        ...
        if (found === -1) {
            diagnostics.push(`sync ${name}: a \`from server\` source must declare a refresh policy: \`every MS\`, \`on KEY\`, or \`once\` (§13.8; liveness is never implied for opaque server code)`);
        ...
        if (/\bthis\b/.test(expr)) {
            diagnostics.push(`sync ${name}: \`this\` cannot cross the server boundary: a server expression has no component instance`);
        }
        ...
        while ((idm = idRe.exec(expr)) !== null) {
            const id = idm[0];
            if (SERVER_EXPR_AMBIENT.has(id))
                continue;
            const after = expr.slice(idm.index + id.length).match(/^\s*:/);
            if (after && !expr.slice(idm.index + id.length).startsWith("::"))
                continue; // object key
            if (importOf.has(id)) {
                hoisted.add(id);
                continue;
            }
            if (componentNames.has(id) && !params.includes(id))
                params.push(id);
    ...
    out += source.slice(last);
    if (sources.length === 0)
        return { code: source, sources, diagnostics };
    ...
    for (const s of sources) {
        ...
        while ((idm = idRe.exec(s.expr)) !== null) {
            if (importOf.has(idm[0]))
                allHoisted.add(idm[0]);
        }
    }
    for (const id of allHoisted) {
        if (schemaRoots.has(id))
            continue;
        if (new RegExp(`(?<![.\\w$])${id}\\b`).test(clientRemainder)) {
            diagnostics.push(`import \`${id}\` (from "${importOf.get(id)}") is used on BOTH sides of the server boundary: split the file, or extract a shared module (§13.8 escape analysis)`);
        }
    }
    ...
    const serverModule = `// AUTO-GENERATED by para-preprocess from ${moduleId}: §13.8 server sources.\n` +
...
function presenceBinding(indent, name, call) {
    return (`${indent}const __pre_${name} = ${call}; ` +
...
function lowerPresenceDecls(source) {
...
function armToDraftFn(entity, arm) {
...
function confirmToFn(arm) {
    return `(${arm.param || "_"}) => { ${arm.body} }`;
}
function lowerMutateDecls(source) {
    ...
    while ((m = re.exec(source)) !== null) {
        ...
        if (!opt)
            continue; // optimistic is required; malformed → leave as-is
        const kwStart = m.index + m[1].length;
        const indent = m[2];
        const name = m[3];
        const entity = m[4];
        ...
        if (rb)
            parts.push(`rollback: ${armToDraftFn(entity, rb)}`);
        if (cf)
            parts.push(`confirm: ${confirmToFn(cf)}`);
...
function lowerAsyncSignalDecls(source) {
    ...
    const code = source.replace(re, (_full, indent, name, expr) => {
        needs = true;
        return (`${indent}const __as_${name} = promiseSignal(() => (${stripDeclTail(expr)})); ` +
 ...
 */
export function buildEscapeChecker(source) {
    ...
    return (name) => {
        if (untraceable)
            return true; // unresolvable signalOf arg → keep bridge for all (safe)
        if (signalOfd.has(name))
            return true;
 ...
 */
export function lowerPuiReactivity(source, runtime = "@lyku/para-ui", linePreserving = false, hmr = false, moduleId) {
    ...
    const svelteImports = new Set(provideInject.imports);
    if (usingResult.needsOnDestroy ||
    ...
    for (const name of PUI_RUNTIME_LIFECYCLE)
        if (usesIdentCall(source, name))
            svelteImports.add(name);
    const kitNavImports = new Set();
    for (const name of PUI_KIT_NAV)
        if (usesIdentCall(source, name))
            kitNavImports.add(name);
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (decls.length === 0 || decls.some(d => d === null))
            continue;
    ...
    for (const name of signalNames) {
        ...
        for (let i = 0; i < lines.length; i++) {
            const line = lines[i];
            if (line.includes(`const __sig_${name}`))
                continue;
    ...
    if (signalNames.size > 0) {
        ...
        if (hmr)
            paraImports.push("hmrSignal");
    }
    if (asyncSignalResult.needsPromiseSignal)
        paraImports.push("promiseSignal");
    if (queryDerivedResult.needsQuerySignal)
        paraImports.push("querySignal");
    ...
    const paraSyncImports = [];
    if (needsSynced || needsServerSync)
        paraSyncImports.push("synced");
    if (needsServerSync)
        paraSyncImports.push("subKey");
    if (needsSyncedQuery)
        paraSyncImports.push("syncedQuery");
    if (syncOneResult.needsSyncedOne)
        paraSyncImports.push("syncedOne");
    if (needsPresence)
        paraSyncImports.push("presence");
    if (needsCreateIntent)
        paraSyncImports.push("createIntent");
    ...
    if (svelteImports.size > 0) {
        ...
        while ((m = importRe.exec(result)) !== null) {
            for (const name of m[1].split(","))
                existing.add(name.trim().split(/\s+as\s+/)[0]);
        }
...
export function parabunPreprocess(opts = {}) {
    ...
    const runtime = opts.runtime ?? "@lyku/para-ui";
    ...
    return {
        ...
        markup({ content, filename }) {
            if (!(filename?.endsWith(".pui") ?? false))
                return;
