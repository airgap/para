// Hardcoded module "@lyku/para-csv"
//
...
//
//   import { parseCsv } from "@lyku/para-csv";
//
...
const parallel = require("@lyku/para-parallel");
...
type ParseOptions = DialectOptions &
  TransformOptions & {
    ...
    /**
     * Opt-in parallel chunk parsing via @lyku/para-parallel's worker pool. Splits
...
async function* tokenize(source: CsvSource, dialect: DialectOptions = {}): AsyncIterable<string[]> {
  ...
  if (Q.length !== 1) throw new TypeError("@lyku/para-csv: quote must be a single character");
  if (E.length !== 1) throw new TypeError("@lyku/para-csv: escape must be a single character");
  if (COMMENT.length > 1) throw new TypeError("@lyku/para-csv: comment must be a single character or empty");
  ...
  if (D === "") {
    ...
    throw new TypeError("@lyku/para-csv: delimiter must be a single character (or empty for auto-detect)");
  }
  ...
  if (state === State.Quoted) {
    throw new SyntaxError("@lyku/para-csv: unterminated quoted field at end of input");
  }
...
//   //   → feed straight into @lyku/para-simd / @lyku/para-arrow with no copy.
//
...
function stringifyImpl(rows: Iterable<StringifyRow>, options: StringifyOptions = {}): string {
  ...
  if (D.length !== 1) throw new TypeError("@lyku/para-csv: delimiter must be a single character");
  if (Q.length !== 1) throw new TypeError("@lyku/para-csv: quote must be a single character");
  if (E.length !== 1) throw new TypeError("@lyku/para-csv: escape must be a single character");
