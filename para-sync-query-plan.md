...
**Follow-up 2026-08-10 (driven by the first private-data consumer, a two-player game with per-user secrets):** four Tier-1 hardening changes, all JS-side, no Zig/parity surface. (a) `createSyncEndpoint` now threads a caller-supplied `ctx` (the v1 "route/host supplies SecureContext" contract made concrete): `GET({url, ctx})` / `POST({request, ctx})`, `onIntent(intent, ctx)`; the emitted endpoint template maps `event.locals → ctx`. (b) `authorize(ctx, key)`: fail-closed per-key read gate; any denied key 403s the whole GET before a byte streams, making per-user subKeys *safe* rather than merely unguessable. Intents are deliberately NOT gated there; `onIntent` owns its own authorization via `ctx`. (c) `project(ctx, key, envelope)` at the send point: per-subscriber shaping on baseline and live frames alike; null suppresses the frame; sequences pass through (they order, they don't authenticate); composes with para-sync's visibility helpers. (d) `heartbeat` (ms) SSE comment frames (`: hb`) so idle turn-based streams survive proxy reaping: invisible to EventSource and `createSseParser` by construction. Plus the reconciler change in para-sync: **gap → direct-commit**: the formerly-documented optimization is now the behavior (a gapped full-object envelope IS the snapshot a refetch would return; same end state as a refetch reseed, @merge base reset included), so reconnect recovery no longer requires the never-shipped snapshot endpoint; `refetch` remains the Err/skew fallback. para-kit 8 new tests (endpoint.test.js), para-sync gap tests rewritten (16 pass), e2e harness updated for the now-async GET.
...
- **Auth surface for `ctx`:** v1 reads `SecureContext` explicitly (endpoint `ctx` param, 2026-08-10); whether P9 auto-threads session → `ctx` is a ch. 11 P8 question, not decided here.
- **Re-subscription for L-key** (§3.4): v2, after auditing shipped `.pui` usage.
- **Sourcemaps in para-preprocess:** still not forwarded (README caveat). Scheduled work, not a sneak-in: it means threading maps through every lowering pass; until then stack traces land on lowered output for `.pui` authors.
