...
**Status:** In progress — steps 1–4 implemented in `packages/para-extract` (2026-07-05): program/checker skeleton, structural lowering (objects/optionals/arrays/literal-union→enum/structural-union→anyOf/template-literal→regex/Date→timestamptz/function marker/null-undefined optionality), recursion → `$ref` with `siblings` linkage and the no-Para-declaration diagnostic, determinism test, and the brand round-trip: `para-schema` constraint brands (`StringOf`/`NumberOf`/`BigIntOf`/`BooleanOf`/`ArrayOf`) re-emit as validator-dialect keywords (`integer: true` → `type: "integer"`, `const` → intersected `enum`, unsafe bigint bounds → strings), resolved through the `parabun` custom condition (the standard variant erases brands). Plain object intersections merge; mapped (`Partial`/`Pick`/literal-key) and conditional types resolve through the checker. Lockstep validator arms (`anyOf`, `function`) in parabun main (2281dcc1bf). Remaining: step 5 (registry linkage via brand markers in codegen'd types), step 6 (`ts<…>` parser/bundler integration).
